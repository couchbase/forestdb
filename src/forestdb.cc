...
INLINE fdb_status _fdb_wal_flush_func(void *voidhandle, struct wal_item *item)
{
    ...
    if (item->action == WAL_ACT_INSERT ||
        ...
        if (hr == HBTRIE_RESULT_SUCCESS) {
            ...
            _kvs_stat_update_attr(file, kv_id, KVS_STAT_NDOCS, -1);
            // Reduce the total datasize by what size of previously present doc
        ...
        if (!docio_check_buffer(handle->dhandle, offset / blocksize)) {
            if (compact_upto &&
                ...
                if (c) {
                    ...
                    _fdb_append_batched_delta(handle, &new_handle, doc, old_offset_array,
                                              c, got_lock, prob, delay_us, do_wal_flush);
                    c = sum_docsize = 0;
                }
                btreeblk_end(handle->bhandle);
