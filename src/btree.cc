    ...
    struct btree *btree, struct bnode *node, void *new_minkey, void *key_arr, void *value_arr, size_t len)
{
    ...
    if (node->flag & BNODE_MASK_METADATA) {
        ...
        nodesize =
            sizeof(struct bnode) +
            btree->kv_ops->getDataSize(node, new_minkey, key_arr, value_arr, len) +
    ...
    struct btree *btree, void *key, void *value)
{
    ...
    btree->kv_ops->initKVVar(item->key, item->value);
    if (key) {
        btree->kv_ops->setKey(item->key, key);
    }
    if (value) {
        btree->kv_ops->setValue(item->value, value);
    }
    return item;
}
    ...
    if (meta) {
        ...
        if (_metasize_align(metasize) < _metasize_align(old_metasize)){
            memmove(
                ...
                node->data,
                btree->kv_ops->getDataSize(node, NULL, NULL, NULL, 0));
...
static idx_t _btree_find_entry(struct btree *btree, struct bnode *node, void *key)
{
    ...
    if (end > 0) {
        ...
        while(start+1 < end) {
            ...
            // get key at middle
            btree->kv_ops->getKV(node, middle, k, NULL);
...
static idx_t _btree_add_entry(struct btree *btree, struct bnode *node, void *key, void *value)
{
    ...
    if (node->nentry > 0) {
        ...
        if (idx_insert < node->nentry) {
            ...
            return 2
            */
            btree->kv_ops->insKV(node, idx_insert, key, value);
        }else{
            btree->kv_ops->setKV(node, idx_insert, key, value);
        }
...
static idx_t _btree_remove_entry(struct btree *btree, struct bnode *node, void *key)
{
    ...
    if (node->nentry > 0) {
        ...
        return 2
        */
        btree->kv_ops->insKV(node, idx, NULL, NULL);
    ...
    struct btree *btree, idx_t num, idx_t den, void *key_begin, void *key_end)
{
    ...
    if (btree->height == 1) {
        btree->kv_ops->getKV(root, ((num+0) * root->nentry / den)-0, key_begin, NULL);
        btree->kv_ops->getKV(root, ((num+1) * root->nentry / den)-1, key_end, NULL);
    }else{
        ...
        btree->kv_ops->getKV(node, idx, key_begin, NULL);
        ...
        btree->kv_ops->getKV(node, idx, key_end, NULL);
    }
...
btree_result btree_find(struct btree *btree, void *key, void *value_buf)
{
    ...
    for (i=btree->height-1; i>=0; --i) {
        ...
        btree->kv_ops->getKV(node[i], idx[i], k, v);
    ...
    int8_t *modified, int8_t *minkey_replace, int8_t *ins)
{
    ...
    // calculate # entry
    for (j=0;j<nnode+1;++j){
        split_idx[j] = btree->kv_ops->getNthIdx(node[i], j, nnode);
        if (j>0) {
    ...
    for (j=1;j<nnode;++j){
        btree->kv_ops->copyKV(new_node[j], node[i], 0, split_idx[j], nentry[j]);
    }
    j = 0;
    btree->kv_ops->copyKV(new_node[j], node[i], 0, split_idx[j], nentry[j]);
    ...
    if (ins[i]) {
        ...
        while(e) {
            ...
            for (j=1;j<nnode;++j){
                btree->kv_ops->getKV(new_node[j], 0, k, v);
    ...
    if (minkey_replace[i]){
        ...
        btree->kv_ops->getKV(new_node[0], idx[i], k, v);
        btree->kv_ops->setKV(new_node[0], idx[i], key, v);
    }
    ...
    if (i+1 < btree->height) {
        ...
        for (j=1; j<nnode; ++j){
            ...
            btree->kv_ops->getNthSplitter(new_node[j-1], new_node[j], kv_item->key);
        ...
        for (j=1;j<nnode;++j){
            ...
            btree->kv_ops->getNthSplitter(new_node[j-1], new_node[j], k);
            _bid = _endian_encode(new_bid[j]);
...
btree_result btree_insert(struct btree *btree, void *key, void *value)
{
    ...
    for (i=0;i<btree->height;++i){
        ...
        if (idx[i] != BTREE_IDX_NOT_FOUND)
            btree->kv_ops->getKV(node[i], idx[i], k, NULL);
        ...
        if (i > 0) {
            ...
            if (moved[i-1]) {
                ...
                btree->kv_ops->setKV(node[i], idx[i], k,
                                     btree->kv_ops->bid2value(&_bid));
                modified[i] = 1;
            }
        }
        ...
        check_node:;
            ...
            if (_size_check || _is_update ) {
                ...
                if (minkey_replace[i]) {
                    ...
                    btree->kv_ops->getKV(node[i], idx[i], k, v);
                    btree->kv_ops->setKV(node[i], idx[i], key, v);
                }
                modified[i] = 1;
    ...
    for (j=0; j < ((height_growup)?(btree->height-1):(btree->height)); ++j){
        ...
        while(e) {
            ...
            btree->kv_ops->freeKVVar(kv_item->key, kv_item->value);
            ...
            */
            if (((node[i]->nentry < 1 && i+1 < btree->height) ||
                ...
                if (i+1 < btree->height) {
                    ...
                    new_rootsize += btree->kv_ops->getDataSize(child, NULL, NULL, NULL, 0);
                                 ...
                                 struct btree_iterator *it, void *initial_key)
{
    ...
    btree->kv_ops->initKVVar(it->curkey, NULL);
    if (initial_key) {
        // set initial key if exists
        //memcpy(it->curkey, initial_key, btree->ksize);
        btree->kv_ops->setKey(it->curkey, initial_key);
    }else{
...
{
    int i;
    it->btree.kv_ops->freeKVVar(it->curkey, NULL);
                                ...
                                void *value_buf, int depth)
{
    ...
    if (it->idx[depth] == BTREE_IDX_NOT_FOUND) {
        ...
        btree->kv_ops->getKV(node, it->idx[depth], key_buf, value_buf);
    ...
    if (depth > 0) {
        ...
        if (it->node[depth-1] == NULL) {
            btree->kv_ops->getKV(node, it->idx[depth], k, v);
        ...
        btree->kv_ops->setKey(it->curkey, key_buf);
        it->idx[depth] = it->idx[depth] ? (it->idx[depth] - 1) :
