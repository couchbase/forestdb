        ...
        if (compress_inmem_stale_info) {
            ...
            if (ret != 0) {
                fdb_log(NULL, FDB_RESULT_COMPRESSION_FAIL,
                    ...
                    "doc offset %" _F64 "\n",
                    file->fileName.c_str(), ret, revnum, doc_offset);
                if (cur == staleInfoTree.end()) {
        ...
        while (offset != BLK_NOT_FOUND) {
            ...
            if (ret <= 0) {
                ...
                fdb_log(NULL, (fdb_status)ret,
                    ...
                    file->fileName.c_str(), revnum, offset);
                             ...
                             bool from_mergetree)
{
    ...
    r = handle->file->kvsStatOps.statGet(0, &stat);
    ...
    handle->file->kvsStatOps.statUpdateAttr(0, KVS_STAT_NLIVENODES, delta);
    delta = handle->bhandle->getNDeltaNodes() - stat.nlivenodes;
    delta *= handle->config.blocksize;
    handle->file->kvsStatOps.statUpdateAttr(0, KVS_STAT_DELTASIZE, delta);
                    ...
                    if (compress_inmem_stale_info) {
                        ...
                        if (r != 0) {
                            fdb_log(NULL, FDB_RESULT_COMPRESSION_FAIL,
                                ...
                                ": return value %d, header revnum %" _F64 ", "
                                "doc offset %" _F64 "\n",
                                handle->file->fileName.c_str(), r, revnum,
                                entry->offset);
                            free(uncomp_buf);
                            free(revnum_array);
                                                                  ...
                                                                  size_t doclen)
{
    ...
    bid_t cur_bid;
    bool non_consecutive = ver_non_consecutive_doc(file->fMgrVersion);
